...
export async function pairCamera(raw:string,deviceFingerprint:string){const response=await fetch(base("/device/pair"),{method:"POST",headers:{Accept:"application/json","Content-Type":"application/json"},body:JSON.stringify({token:raw,deviceFingerprint,platform:Platform.OS,appVersion:"0.2.0",capabilities:["video-capture","qr-pairing","heartbeat","local-buffer-ready"]})});return data<CameraRegistration>(response)}
...
export async function cameraHeartbeat(reg:CameraRegistration){return data<{ok:boolean;serverId?:string|null;serverAssigned:boolean;serverTime:string}>(await fetch(base("/device/heartbeat"),{method:"POST",headers:headers(reg,true),body:JSON.stringify({appVersion:"0.2.0",platform:Platform.OS})}))}
