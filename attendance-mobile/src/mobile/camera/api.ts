...
export async function pairCamera(raw:string,deviceFingerprint:string){const response=await fetch(base("/device/pair"),{method:"POST",headers:{Accept:"application/json","Content-Type":"application/json"},body:JSON.stringify({token:raw,deviceFingerprint,platform:Platform.OS,appVersion:"0.6.0",capabilities:["video-capture","qr-pairing","heartbeat","whip-publish","whep-live","segmented-mp4-fallback","offline-segment-queue","android-appliance","remote-capture-profile"]})});return data<CameraRegistration>(response)}
