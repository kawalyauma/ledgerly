...
export function KioskScreen({registration,onReset}:{registration:Registration;onReset:()=>void}){
  ...
  return <SafeAreaView style={s.root}>
    ...
    {job?<View style={s.enroll}><View><Text style={s.enrollTitle}>Face enrollment requested</Text><Text style={s.enrollText}>{job.personName}{job.groupName?` · ${job.groupName}`:""}</Text></View><TouchableOpacity disabled={faceBusy} style={s.enrollButton} onPress={()=>void enroll(job)}><Text style={s.enrollButtonText}>{faceBusy?"Working…":"Start enrollment"}</Text></TouchableOpacity></View>:null}
    <View style={s.camera}>
      ...
      <View pointerEvents="none" style={s.guide}><View style={s.faceGuide}/><Text style={s.guideText}>{facePrompt||"Use the device camera for face or card capture"}</Text></View>
    ...
    <View style={s.methods}><TouchableOpacity disabled={faceBusy} style={[s.faceButton,faceBusy&&s.disabled]} onPress={()=>void face()}><Text style={s.faceButtonText}>{faceBusy?"Face capture…":"Scan face"}</Text></TouchableOpacity><TouchableOpacity disabled={bootstrap?.device.direction!=="BOTH"} style={s.direction} onPress={()=>setDirection(x=>x==="IN"?"OUT":"IN")}><Text style={s.directionLabel}>DIRECTION</Text><Text style={s.directionValue}>{direction}</Text></TouchableOpacity></View>
