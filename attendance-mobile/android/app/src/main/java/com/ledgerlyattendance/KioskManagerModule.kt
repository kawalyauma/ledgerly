...
class KioskManagerModule(private val context:ReactApplicationContext):ReactContextBaseJavaModule(context){
  ...
  @ReactMethod fun enter(promise:Promise){val activity=context.currentActivity?:return promise.reject("NO_ACTIVITY","No active kiosk activity");try{if(dpm().isDeviceOwnerApp(context.packageName))dpm().setLockTaskPackages(android.content.ComponentName(context,LedgerlyDeviceAdminReceiver::class.java),arrayOf(context.packageName));activity.startLockTask();activity.window.decorView.systemUiVisibility=View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY or View.SYSTEM_UI_FLAG_FULLSCREEN or View.SYSTEM_UI_FLAG_HIDE_NAVIGATION;promise.resolve(true)}catch(e:Exception){promise.reject("KIOSK_ENTER_FAILED",e)}}
