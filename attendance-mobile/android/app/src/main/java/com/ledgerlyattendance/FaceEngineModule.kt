...
class FaceEngineModule(private val context:ReactApplicationContext):ReactContextBaseJavaModule(context){
  ...
  private fun crop(sample:FaceSample):Bitmap{
    val b=sample.face.boundingBox;val bmp=sample.bitmap;val margin=(max(b.width(),b.height())*.20).toInt();val size=max(b.width(),b.height())+2*margin
    ...
    val faceBmp=Bitmap.createBitmap(bmp,left,top,(right-left).coerceAtLeast(1),(bottom-top).coerceAtLeast(1))
  ...
  @ReactMethod fun healthCheck(promise:Promise){executor.execute{try{engine();promise.resolve(Arguments.createMap().apply{putBoolean("healthy",true);putString("provider","facenet-128-v1");putBoolean("livenessRequired",true);putInt("templateCount",store.count());putString("modelError",null)})}catch(e:Throwable){promise.resolve(Arguments.createMap().apply{putBoolean("healthy",false);putString("provider","facenet-128-v1");putBoolean("livenessRequired",true);putInt("templateCount",store.count());putString("modelError",modelError?:e.message)})}}}
  @ReactMethod fun replaceTemplates(json:String,promise:Promise){executor.execute{try{val a=JSONArray(json);val rows=mutableListOf<TemplateInput>();for(i in 0 until a.length()){val x=a.getJSONObject(i);rows+=TemplateInput(x.getString("personType"),x.getString("personId"),x.getString("algorithmVersion"),x.getString("embeddingBase64"),x.optString("updatedAt",null))};store.replaceAll(rows);promise.resolve(rows.size)}catch(e:Throwable){promise.reject("FACE_TEMPLATE_SYNC_FAILED",e.message,e)}}}
  @ReactMethod fun inspect(imagePath:String,promise:Promise){executor.execute{var s:FaceSample?=null;try{s=sample(imagePath);promise.resolve(Arguments.createMap().apply{putDouble("quality",s!!.quality);putDouble("yaw",s!!.face.headEulerAngleY.toDouble());putDouble("roll",s!!.face.headEulerAngleZ.toDouble());putDouble("leftEyeOpen",(s!!.face.leftEyeOpenProbability?:-1f).toDouble());putDouble("rightEyeOpen",(s!!.face.rightEyeOpenProbability?:-1f).toDouble())})}catch(e:Throwable){promise.reject("FACE_INSPECTION_FAILED",e.message,e)}finally{s?.let{clean(listOf(it))}}}} 
  @ReactMethod fun enroll(imagePaths:ReadableArray,challenge:String,promise:Promise){executor.execute{val ss=mutableListOf<FaceSample>();try{for(p in paths(imagePaths))ss+=sample(p);if(ss.size<3)throw IllegalStateException("Enrollment requires three live camera samples");val quality=ss.map{it.quality}.average();if(quality<qualityThreshold)throw IllegalStateException("Face quality is too low. Improve lighting and move closer to the camera.");val live=liveness(ss,challenge);if(live<livenessThreshold)throw IllegalStateException("Liveness challenge failed. Follow the on-screen instruction and hold the requested pose until capture.");val emb=average(ss);promise.resolve(Arguments.createMap().apply{putString("algorithmVersion","facenet-128-v1");putString("embeddingBase64",FaceEncoding.encodeEmbedding(emb));putDouble("qualityScore",quality);putDouble("livenessScore",live);putInt("poseCount",ss.size)})}catch(e:Throwable){promise.reject("FACE_ENROLLMENT_FAILED",e.message,e)}finally{clean(ss)}}}
  @ReactMethod fun identify(imagePaths:ReadableArray,testMode:Boolean,allowScreenImage:Boolean,allowPrintedImage:Boolean,challenge:String,promise:Promise){executor.execute{val ss=mutableListOf<FaceSample>();try{for(p in paths(imagePaths))ss+=sample(p);if(ss.isEmpty())throw IllegalStateException("No usable face sample");val quality=ss.map{it.quality}.average();if(quality<qualityThreshold)throw IllegalStateException("Face quality is too low. Move closer and improve lighting.");val bypass=testMode&&(allowScreenImage||allowPrintedImage);val live=if(bypass)0.0 else liveness(ss,challenge);if(!bypass&&live<livenessThreshold)throw IllegalStateException("Liveness challenge failed");val emb=average(ss);var best:StoredFaceTemplate?=null;var bestScore=-1.0;var secondScore=-1.0;for(t in store.all()){if(t.algorithmVersion!="facenet-128-v1")continue;val x=cosine(emb,t.embedding);if(x>bestScore){secondScore=bestScore;bestScore=x;best=t}else if(x>secondScore){secondScore=x}};if(best==null)throw IllegalStateException("No enrolled face templates are available on this kiosk");if(bestScore<matchThreshold)throw IllegalStateException("Face not recognized with enough confidence");val margin=if(secondScore<0)1.0 else bestScore-secondScore;if(margin<ambiguityMargin)throw IllegalStateException("Face match is ambiguous. Use QR, NFC or supervised lookup.");promise.resolve(Arguments.createMap().apply{putString("personId",best!!.personId);putString("personType",best!!.personType);putDouble("confidence",bestScore);putDouble("secondBestConfidence",secondScore.coerceAtLeast(0.0));putDouble("matchMargin",margin);putDouble("livenessScore",live);putBoolean("testMode",testMode)})}catch(e:Throwable){promise.reject("FACE_NOT_RECOGNIZED",e.message,e)}finally{clean(ss)}}}
  @ReactMethod fun verify(personType:String,personId:String,imagePaths:ReadableArray,testMode:Boolean,allowScreenImage:Boolean,allowPrintedImage:Boolean,challenge:String,promise:Promise){executor.execute{val ss=mutableListOf<FaceSample>();try{for(p in paths(imagePaths))ss+=sample(p);if(ss.isEmpty())throw IllegalStateException("No usable face sample");val quality=ss.map{it.quality}.average();if(quality<qualityThreshold)throw IllegalStateException("Face quality is too low");val bypass=testMode&&(allowScreenImage||allowPrintedImage);val live=if(bypass)0.0 else liveness(ss,challenge);if(!bypass&&live<livenessThreshold)throw IllegalStateException("Liveness challenge failed");val target=store.all().firstOrNull{it.personType==personType&&it.personId==personId&&it.algorithmVersion=="facenet-128-v1"}?:throw IllegalStateException("This person has no face template on this kiosk");val score=cosine(average(ss),target.embedding);promise.resolve(Arguments.createMap().apply{putBoolean("matched",score>=matchThreshold);putDouble("confidence",score);putDouble("livenessScore",live);putBoolean("testMode",testMode)})}catch(e:Throwable){promise.reject("FACE_VERIFY_FAILED",e.message,e)}finally{clean(ss)}}}
}
