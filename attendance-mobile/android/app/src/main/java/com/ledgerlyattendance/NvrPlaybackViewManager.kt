...
class NvrPlaybackView(context: ThemedReactContext) : FrameLayout(context) {
  ...
  fun setSource(next: String?) {
    ...
    video.setOnPreparedListener { preparedPlayer ->
      ...
      preparedPlayer.setOnInfoListener { _, what, _ ->
        if (generation != sourceGeneration) return@setOnInfoListener false
        when (what) {
    ...
    video.setOnCompletionListener {
      if (generation == sourceGeneration) emit("ended")
    }
    video.setOnErrorListener { _, what, extra ->
      if (generation != sourceGeneration) return@setOnErrorListener true
      prepared = false
      player = null
      emit("error", "Media error $what/$extra")
      true
    }
  ...
  fun setPosition(next: Int) {
    desiredPositionMs = next.coerceAtLeast(0)
    if (prepared && abs(video.currentPosition - desiredPositionMs) > 700) {
      video.seekTo(desiredPositionMs)
    }
  }

  fun setRate(next: Float) {
    desiredRate = next.coerceIn(.25f, 4f)
    if (prepared) applyRate(player)
  }

  private fun applyRate(target: MediaPlayer?) {
    if (target != null && Build.VERSION.SDK_INT >= 23) {
      try {
        target.playbackParams = target.playbackParams.setSpeed(desiredRate)
        if (!shouldPlay) target.pause()
      } catch (_: Throwable) {}
    }
  }
  ...
  private fun emit(state: String, message: String? = null) {
    ...
    (context as ThemedReactContext)
      ...
      .receiveEvent(id, "topNvrPlaybackState", event)
  }
...
class NvrPlaybackViewManager : SimpleViewManager<NvrPlaybackView>() {
  ...
  fun source(view: NvrPlaybackView, value: String?) {
    view.setSource(value)
  }

  @ReactProp(name = "playing", defaultBoolean = false)
  fun playing(view: NvrPlaybackView, value: Boolean) {
    view.setPlaying(value)
  }

  @ReactProp(name = "positionMs", defaultInt = 0)
  fun position(view: NvrPlaybackView, value: Int) {
    view.setPosition(value)
  }

  @ReactProp(name = "rate", defaultFloat = 1f)
  fun rate(view: NvrPlaybackView, value: Float) {
    view.setRate(value)
  }

  override fun getExportedCustomDirectEventTypeConstants(): MutableMap<String, Any> =
    MapBuilder.of("topNvrPlaybackState", MapBuilder.of("registrationName", "onNvrPlaybackState"))

  override fun onDropViewInstance(view: NvrPlaybackView) {
