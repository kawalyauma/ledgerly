...
import {AudioCallsWorkspaceScreenV2,type MobileIncomingCall} from "./src/mobile/audio-calls/AudioCallsWorkspaceScreenV2";
import {GlobalIncomingCallOverlay} from "./src/mobile/audio-calls/GlobalIncomingCallOverlay";
...
export default function App(){
  const[ready,setReady]=useState(false),[onboarded,setOnboarded]=useState(false),[session,setSession]=useState<MobileSession|null>(null),[purpose,setPurpose]=useState<DevicePurpose|null>(null),[route,setRoute]=useState<Route>("home"),[attendanceView,setAttendanceView]=useState<"landing"|"workspace"|"register"|"kiosk">("landing"),[registration,setRegistration]=useState<Registration|null|undefined>(undefined),[pendingIncoming,setPendingIncoming]=useState<MobileIncomingCall|null>(null);
  ...
  function requestLogout(){Alert.alert("Sign out?","You will need your Ledgerly credentials to sign in again.",[{text:"Cancel",style:"cancel"},{text:"Sign out",style:"destructive",onPress:()=>{const current=session;setSession(null);setRoute("home");setPendingIncoming(null);void Promise.all([clearMobileSession(),clearMobileSyncAccountData()]);if(current)void logoutMobile(current)}}])}
  function withCalls(view:ReactNode){if(!session)return view;return <View style={s.stack}>{view}<GlobalIncomingCallOverlay session={session} onSession={updateSession} enabled={route!=="audio-calls"} onOpen={call=>{setPendingIncoming(call);setRoute("audio-calls")}}/></View>}
  ...
  if(purpose==="attendance-kiosk"){if(registration===undefined)return <View style={s.loading}><ActivityIndicator color="#55c894"/></View>;if(!registration)return <ActivationScreen onActivated={(next:Registration)=>{setRegistration(next);setAttendanceView("kiosk")}}/>;return <><StatusBar hidden/><KioskScreen registration={registration} onReset={()=>{setRegistration(null);setAttendanceView("register")}}/></>}
  if(!session)return <LoginScreen onLogin={signedIn}/>;
  if(route==="school")return withCalls(<SchoolWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="academics")return withCalls(<AcademicsWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="exams")return withCalls(<ExamsWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="books")return withCalls(<BooksWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="human-resources")return withCalls(<HumanResourcesWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="payroll-payments")return withCalls(<PayrollPaymentsWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="contacts")return withCalls(<ContactsWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="communications")return withCalls(<CommunicationsWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="tasks-work")return withCalls(<TasksWorkWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="finance-core")return withCalls(<FinanceCoreWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="printerly")return withCalls(<PrinterlyWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="security-cameras")return withCalls(<SecurityCamerasWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setRoute("home")}/>);
  if(route==="audio-calls")return withCalls(<AudioCallsWorkspaceScreenV2 session={session} onSession={updateSession} initialIncoming={pendingIncoming} onBack={()=>{setPendingIncoming(null);setRoute("home")}}/>);
  if(route==="attendance"){if(registration===undefined)return <View style={s.loading}><ActivityIndicator color="#55c894"/></View>;if(attendanceView==="register")return withCalls(<ActivationScreen onActivated={(next:Registration)=>{setRegistration(next);setAttendanceView("landing")}}/>);if(attendanceView==="kiosk"&&registration)return <><StatusBar hidden/><KioskScreen registration={registration} onReset={()=>{setRegistration(null);setAttendanceView("landing")}}/></>;if(attendanceView==="workspace")return withCalls(<AttendanceWorkspaceScreen session={session} onSession={updateSession} onBack={()=>setAttendanceView("landing")} onOpenKiosk={()=>setAttendanceView(registration?"kiosk":"register")}/>);return withCalls(<AttendanceModuleScreen registration={registration} onBack={()=>setRoute("home")} onWorkspace={()=>setAttendanceView("workspace")} onRegister={()=>setAttendanceView("register")} onLaunch={()=>setAttendanceView("kiosk")}/>)}
  return withCalls(<HomeScreen session={session} onSession={updateSession} onAttendance={()=>{setAttendanceView("landing");setRoute("attendance")}} onSchool={()=>setRoute("school")} onAcademics={()=>setRoute("academics")} onExams={()=>setRoute("exams")} onBooks={()=>setRoute("books")} onHumanResources={()=>setRoute("human-resources")} onPayrollPayments={()=>setRoute("payroll-payments")} onContacts={()=>setRoute("contacts")} onCommunications={()=>setRoute("communications")} onTasksWork={()=>setRoute("tasks-work")} onFinanceCore={()=>setRoute("finance-core")} onPrinterly={()=>setRoute("printerly")} onSecurityCameras={()=>setRoute("security-cameras")} onAudioCalls={()=>{setPendingIncoming(null);setRoute("audio-calls")}} onLogout={requestLogout}/>);
}
const s=StyleSheet.create({stack:{flex:1},loading:{flex:1,backgroundColor:"#071c16",alignItems:"center",justifyContent:"center"},loaderMark:{width:58,height:58,borderRadius:18,backgroundColor:"#19955f",alignItems:"center",justifyContent:"center"},loaderLetter:{color:"white",fontSize:30,fontWeight:"900"},loadingText:{color:"#9fb9ae",fontSize:12,fontWeight:"800",marginTop:10}});
