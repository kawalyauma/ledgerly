...
securityCameraRedundancyRoutes.get("/redundancy",...read,async c=>c.json({data:{configs:await R.listRedundancy(c.env.FINANCE_DB,c.get("principal").organizationId),events:await R.listFailoverEvents(c.env.FINANCE_DB,c.get("principal").organizationId,200),validation:await R.validationSummary(c.env.FINANCE_DB,c.get("principal").organizationId)}}));
