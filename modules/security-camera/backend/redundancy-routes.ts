...
securityCameraRedundancyRoutes.get("/redundancy",...read,async c=>{const org=c.get("principal").organizationId;const[configs,events,validation,backups]=await Promise.all([R.listRedundancy(c.env.FINANCE_DB,org),R.listFailoverEvents(c.env.FINANCE_DB,org,200),R.validationSummary(c.env.FINANCE_DB,org),listBackupStatus(c.env.FINANCE_DB,org)]);return c.json({data:{configs,events,validation,backups}})});
