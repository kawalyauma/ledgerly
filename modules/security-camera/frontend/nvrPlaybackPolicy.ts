...
export function nextCoveredTime(groups:NvrSegment[][],timeMs:number,toMs:number){if(!groups.length||groups.some(group=>!group.length))return null;if(groups.some(group=>segmentAt(group,timeMs)))return timeMs;let next=Number.POSITIVE_INFINITY;for(const group of groups)for(const segment of group){const start=Date.parse(segment.started_at);if(Number.isFinite(start)&&start>timeMs&&start<=toMs)next=Math.min(next,start)}return Number.isFinite(next)?next:null}
