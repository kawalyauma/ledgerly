...
import { BellOff, Clock3, Mic, MicOff, Phone, PhoneOff, RefreshCw, Search, ShieldCheck, Wifi } from "lucide-react";
...
import { getCallAvailability, setCallAvailability, subscribeCallAvailability, type CallAvailability } from "./presencePreference";
...
export function AudioCallsWorkspace(){
 const[workers,setWorkers]=useState<WorkerPresence[]>([]),[recent,setRecent]=useState<RecentCall[]>([]),[query,setQuery]=useState(""),[loading,setLoading]=useState(true),[error,setError]=useState(""),[state,setState]=useState<CallState>(audioCallEngine.snapshot()),[tick,setTick]=useState(0),[availability,setAvailabilityState]=useState<CallAvailability>(getCallAvailability());
 useEffect(()=>audioCallEngine.subscribe(setState),[]);
 useEffect(()=>subscribeCallAvailability(setAvailabilityState),[]);
 ...
 async function changeAvailability(value:CallAvailability){setCallAvailability(value);setAvailabilityState(value);try{await post("/audio-calls/presence/heartbeat",{availability:value,deviceId:"web"})}catch(e){setError(errorText(e))}}
 return <div className="audio-calls-page">
  <section className="audio-calls-hero">
   ...
   <div className="audio-calls-trust"><ShieldCheck size={20}/><span><b>No audio recording</b><small>Only call metadata and short-lived signaling are stored.</small></span></div>
  </section>

  <section className="audio-call-availability" aria-label="Call availability"><div><BellOff size={17}/><span><b>Incoming calls</b><small>{availability==="do_not_disturb"?"Do Not Disturb is on. New calls are blocked by the server.":"You are available for Ledgerly calls."}</small></span></div><div className="audio-call-availability-actions"><button className={availability==="available"?"active":""} disabled={active} onClick={()=>void changeAvailability("available")}>Available</button><button className={availability==="do_not_disturb"?"active":""} disabled={active} onClick={()=>void changeAvailability("do_not_disturb")}>Do not disturb</button></div></section>
  ...
  <footer className="audio-call-network-note"><Wifi size={17}/><span>For reliable calls across mobile networks, NAT and firewalls, configure the Audio Calls TURN secret on the Worker.</span></footer>
 </div>
}
