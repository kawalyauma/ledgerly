...
export function acceptedStaleSeconds(){return 90;}
...
export async function expireStaleCalls(env:Env,limit=250){
  ...
  for(const call of accepted.results){
    const changed=await db.prepare(`UPDATE audio_calls SET status='failed',ended_at=CURRENT_TIMESTAMP,end_reason='participant_disconnected',updated_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=? AND status='accepted'`).bind(call.id,call.organizationId).run();
    if(changed.meta.changes)await releaseCallState(db,call.organizationId,call.id);
  }
