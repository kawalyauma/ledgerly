...
const ensureSchoolBase=async(database)=>{await ensureAuthCoreSchema(database);await ensureReferenceWithLateColumns(database);await ensureSchoolConfigurationSchema(database);await ensureSchoolFeeReferenceSchema(database);};
...
const PHASES=Object.freeze({
 ...
 "school-configuration":Object.freeze({name:"school-configuration",description:"School grading, calendar, fee/payment references, settings, templates, files and school-scoped IAM",prerequisites:Object.freeze(["auth-core","school-reference"]),tables:Object.freeze([...SCHOOL_CONFIGURATION_TABLES,...SCHOOL_FEE_REFERENCE_TABLES]),ensureSchema:ensureSchoolBase,finalizeSchema:finalizeConfiguration,relationshipChecks:Object.freeze([...SCHOOL_CONFIGURATION_RELATIONSHIP_CHECKS,...SCHOOL_FEE_REFERENCE_RELATIONSHIP_CHECKS])}),
