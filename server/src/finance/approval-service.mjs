...
export class FinanceApprovalService{
  ...
  async createPolicy({principal:p,body={},requestId}){
    ...
    const row=(await this.db.query(`INSERT INTO approval_policies(id,organization_id,document_type,minimum_minor,maximum_minor,levels,approver_roles,active,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,true,now(),now()) RETURNING *`,[id,p.organizationId,documentType,minimum.toString(),maximum?.toString()??null,levels,JSON.stringify(roles)])).rows[0];
    await this.invalidate(p.organizationId,'reference-counts');await audited(this.services,p,requestId,'finance.approval-policy.create','approval-policy',id,row);return row;
  }
  ...
  async submitDocument({principal:p,documentId,requestId}){
    ...
    const result=await this.db.transaction(async tx=>{
      ...
      if(document.approval_status==='pending'){
        const existing=(await tx.query(`SELECT * FROM document_approval_requests WHERE organization_id=$1 AND entity_type='document' AND entity_id=$2 AND status='pending' ORDER BY submitted_at DESC LIMIT 1 FOR UPDATE`,[org,id])).rows[0];
      ...
      const policy=(await tx.query(`SELECT * FROM approval_policies WHERE organization_id=$1 AND document_type=$2 AND active=true AND minimum_minor<=$3 AND (maximum_minor IS NULL OR maximum_minor>=$3) ORDER BY levels DESC,id LIMIT 1 FOR UPDATE`,[org,document.type,document.total_minor])).rows[0];
      if(!policy)throw error(409,'NO_APPROVAL_POLICY','No applicable approval policy');
      const requestIdValue=randomUUID();
      const row=(await tx.query(`INSERT INTO document_approval_requests(id,organization_id,entity_type,entity_id,policy_id,status,current_level,submitted_by,submitted_at,created_at,updated_at) VALUES($1,$2,'document',$3,$4,'pending',1,$5,now(),now(),now()) RETURNING *`,[requestIdValue,org,id,policy.id,p.userId])).rows[0];
  ...
  async decide({principal:p,approvalId,decision,body={},requestId}){
    ...
    const result=await this.db.transaction(async tx=>{
      const request=(await tx.query(`SELECT r.*,d.id AS document_id,d.approval_status FROM document_approval_requests r JOIN documents d ON d.id=r.entity_id AND d.organization_id=r.organization_id WHERE r.id=$1 AND r.organization_id=$2 AND r.entity_type='document' FOR UPDATE OF r,d`,[id,org])).rows[0];
      ...
      const updated=(await tx.query(`UPDATE document_approval_requests SET status=$3,decided_by=$4,decided_at=now(),comments=$5,updated_at=now() WHERE id=$1 AND organization_id=$2 AND status='pending' RETURNING *`,[id,org,status,p.userId,comments])).rows[0];
      if(!updated)throw error(409,'CONCURRENT_APPROVAL_CHANGE','Approval changed while deciding');
      const document=await tx.query(`UPDATE documents SET approval_status=$3,updated_at=now() WHERE id=$1 AND organization_id=$2 RETURNING id,approval_status`,[request.document_id,org,status]);
