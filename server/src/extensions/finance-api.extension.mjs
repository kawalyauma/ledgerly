...
export default{
 ...
 enabled(){return true},
 async create({services,extensionConfig}){const shared={database:services.database,cache:services.cache};return{value:Object.freeze({accounts:new FinanceAccountsService(shared),journals:new FinanceJournalsService(shared),payments:new FinancePaymentsService(shared),...extensionConfig}),async readiness(){return{ok:true,...extensionConfig}},describe(){return{...extensionConfig,cloudflareFallback:true,granularRoutes:true}}}}
};
