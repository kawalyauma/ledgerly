...
export default{
  ...
  configure(env){return{cutover:mode(env.LEDGERLY_PRINTERLY_CUTOVER),outboxPollMs:Math.max(500,Number(env.LEDGERLY_PRINTERLY_OUTBOX_POLL_MS)||2000),workerPollMs:Math.max(250,Number(env.LEDGERLY_PRINTERLY_WORKER_POLL_MS)||1000)};},
  enabled(){return true;},
  async create({services,createQueue,extensionConfig}){
    ...
    if(extensionConfig.cutover!=='cloudflare'){
      outboxTimer=setInterval(()=>void drainOutbox(),extensionConfig.outboxPollMs);outboxTimer.unref?.();
      workerTimer=setInterval(()=>void drainQueue(),extensionConfig.workerPollMs);workerTimer.unref?.();
    ...
    return{
      ...
      async readiness(){
        ...
        const [deadOutbox,deadQueue]=await Promise.all([services.database.query(`SELECT COUNT(*)::int count FROM prn_dispatch_outbox WHERE status='dead'`),queue.deadLetterSize()]);
        return{ok:Number(deadOutbox.rows[0]?.count||0)===0&&Number(deadQueue||0)===0&&!scheduleState.error,cutover:extensionConfig.cutover,authoritative,deadOutbox:Number(deadOutbox.rows[0]?.count||0),deadQueue:Number(deadQueue||0),schedules:scheduleState};
      },
      describe(){return{cutover:extensionConfig.cutover,legacyNodeApi:authoritative,outbox:'postgres-to-redis',worker:extensionConfig.cutover==='cloudflare'?'disabled':'redis-consumer',schedules:scheduleState};},
