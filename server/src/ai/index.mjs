...
export async function createAiWorkforce({services,config={},businessTools={},authorization=null,tenantStorage=null,logger=console}){
  ...
  const providerConfig={
    ...
    contextLimit:Number(config.contextLimit??8192),
    temperature:Number(config.temperature??0.2),
    toolSupport:config.toolSupport!==false,
  };
  ...
  const embedder=embeddingModel?{
    async embed(text){
      const provider=providerRegistry.create(config.provider??"ollama",{...providerConfig,model:embeddingModel,toolSupport:false});
      return provider.embed(String(text??""));
    },
  }:null;
  ...
  const ingestion=new AiKnowledgeIngestionService({
    knowledge,
    storageForContext:(context)=>tenantStorage?.forOrganization(context.organizationId),
    audit:services.audit,
    maxBytes:Number(config.maxKnowledgeBytes??25*1024*1024),
    chunkChars:Number(config.chunkChars??3200),
    overlapChars:Number(config.chunkOverlapChars??400),
  });
  ...
  async function uploadKnowledge({context,name,bytes,contentType="application/octet-stream",sourceType="uploaded_document",metadata={}}){
    if(!tenantStorage?.forOrganization)throw new Error("Tenant storage is unavailable");
    const storage=tenantStorage.forOrganization(context.organizationId);
    const key=`ai/knowledge/${randomUUID()}-${safeName(name)}`;
    await storage.put(key,Buffer.from(bytes),{contentType,custom:{uploaded_by:String(context.userId),source_type:String(sourceType)}});
    return ingestion.ingestStored({context,name,sourceType,storageRef:key,contentType,metadata});
  }

  async function getRenderedDocumentUrl({context,documentId,expiresSeconds=900}){
    if(!tenantStorage?.forOrganization)throw new Error("Tenant storage is unavailable");
    const result=await services.database.query(`SELECT rendered_pdf_ref FROM ledgerly_ai.documents WHERE document_id=$1 AND organization_id=$2`,[documentId,context.organizationId]);
    const ref=result.rows[0]?.rendered_pdf_ref;
    if(!ref)throw new Error("Document has no rendered PDF");
    return {ref,url:await tenantStorage.forOrganization(context.organizationId).createDownloadUrl(ref,{expiresSeconds})};
  }
  ...
  async function health(){
    ...
    catch(error){runtime={ok:false,provider:providerId,state:"error",error:error instanceof Error?error.message:String(error),code:error?.code};}
    const [queueLength,taskStats]=await Promise.all([
      services.queue.size().catch(()=>null),
      services.database.query(`SELECT count(*) FILTER (WHERE status='failed')::int AS failed,
        count(*) FILTER (WHERE status='working')::int AS working,
        avg(EXTRACT(EPOCH FROM (completed_at-created_at))*1000) FILTER (WHERE completed_at IS NOT NULL) AS avg_ms,
        max(completed_at) AS last_success
        FROM ledgerly_ai.tasks`).then((r)=>r.rows[0]??{}).catch(()=>({})),
    ]);
