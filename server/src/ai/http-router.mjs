...
export async function handleAiRequest({request,url,runtime}){
  ...
  if(parts[0]==="knowledge"){
    ...
    if(method==="POST"&&parts[1]==="upload"){const bytes=await readRaw(request,MAX_UPLOAD_BYTES);if(!bytes.length){const e=new Error("Knowledge upload is empty");e.status=422;throw e;}const name=cleanFilename(request.headers["x-file-name"]||url.searchParams.get("name")||"knowledge-document"),contentType=String(request.headers["content-type"]||"application/octet-stream").split(";")[0].trim(),sourceType=String(request.headers["x-source-type"]||"uploaded_document");return json(201,await runtime.ai.uploadKnowledge({context:ctx,name,bytes,contentType,sourceType}));}
