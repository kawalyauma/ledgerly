...
async function body(request){let size=0;const chunks=[];for await(const chunk of request){size+=chunk.length;if(size>MAX_BODY_BYTES){const e=new Error("Request body too large");e.status=413;throw e;}chunks.push(chunk);}if(!chunks.length)return{};try{return JSON.parse(Buffer.concat(chunks).toString("utf8"));}catch{const e=new Error("Invalid JSON body");e.status=400;throw e;}}
...
export async function handleAiRequest({request,url,runtime}){
 ...
 if(parts[0]==="tasks"){
  ...
  if(method==="POST"&&parts[2]==="handoff"){const task=(await db.query(`SELECT * FROM ledgerly_ai.tasks WHERE task_id=$1 AND organization_id=$2`,[parts[1],ctx.organizationId])).rows[0];if(!task)return json(404,{error:{code:"AI_TASK_NOT_FOUND",message:"AI task not found"}});const fromAgent=await runtime.ai.agents.get(ctx,task.assigned_agent);if(!fromAgent)return json(409,{error:{code:"AI_HANDOFF_SOURCE_MISSING",message:"Source AI employee no longer exists"}});const input=await body(request);return json(201,await runtime.ai.tasks.handoff({context:ctx,task,fromAgent,toAgent:input.toAgent,instruction:input.instruction}));}
 ...
 if(parts[0]==="approvals"){
  ...
  if(method==="POST"&&parts[2]==="execute"){runtime.auth.requireScope(principal,"ai:approve");const approval=await runtime.ai.approvals.get({context:ctx,approvalId:parts[1]});if(!approval)return json(404,{error:{code:"AI_APPROVAL_NOT_FOUND",message:"Approval not found"}});const agent=await runtime.ai.agents.get(ctx,approval.agent_id);if(!agent)return json(409,{error:{code:"AI_APPROVAL_AGENT_MISSING",message:"Approval agent no longer exists"}});const execution=await runtime.ai.gateway.executeApproved({approval,agent,context:ctx});const saved=await runtime.ai.approvals.markExecuted({context:ctx,approvalId:parts[1],result:execution.output});if(approval.task_id)await runtime.ai.tasks.setStatus({organizationId:ctx.organizationId,taskId:approval.task_id,status:"completed",agent,outputReferences:[{type:"approved_action",approvalId:approval.approval_id,result:execution.output}]});return json(200,{approval:saved,execution});}
 }
 ...
 if(parts[0]==="settings"&&method==="GET")return json(200,{provider:runtime.ai.config.providerConfig,limits:runtime.ai.config.limits,knowledge:runtime.ai.storeCapabilities});
