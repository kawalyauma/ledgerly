...
export class AiToolGateway {
  ...
  describeForAgent(agent) {
    return [...this.tools.values()]
      .filter((tool) => (agent.allowedTools ?? []).includes(tool.name))
  ...
  async invoke({ agent, context, taskId = null, toolName, input = {} }) {
    ...
    await this.audit?.write?.({
      ...
      actor_type:"ai_agent", actor_id:agent.agentId,
      agent_id:agent.agentId, agent_name:agent.name, agent_role:agent.role,
      action:"ai.tool.requested", entity_type:"ai_tool", entity_id:toolName,
      reason:context.reason ?? null,
      metadata:{ task_id:taskId, policy, input:tool.auditInput === false ? undefined : input },
    });
    ...
    if (policy.decision === "approval_required") {
      ...
      const approval = await this.approvalService.request({
        ...
        agent, taskId, action:toolName, reason:context.reason ?? policy.reason,
        payload:input, riskLevel:policy.risk,
    ...
    const output = await tool.handler({ input, context, agent, taskId });
    await this.audit?.write?.({
      organization_id:context.organizationId,
      actor_type:"ai_agent", actor_id:agent.agentId,
      agent_id:agent.agentId, agent_name:agent.name, agent_role:agent.role,
      action:"ai.tool.executed", entity_type:"ai_tool", entity_id:toolName,
      reason:context.reason ?? null,
      metadata:{ task_id:taskId, risk:policy.risk },
    });
    return { status:"executed", output, policy };
  }
}
...
export function registerCoreTools(gateway, services = {}) {
  ...
  for (const name of ["delete_student","alter_marks","reverse_journal","post_payroll","post_payment","alter_financial_record","adjust_stock"]) {
    gateway.register({ name, permissions:["ai:restricted"], risk:"prohibited", consequential:true, approvalRequired:true }, unavailable(name));
  }
  return gateway;
}

function unavailable(name) { return async () => { throw new Error(`Ledgerly business adapter for ${name} is not connected yet`); }; }
