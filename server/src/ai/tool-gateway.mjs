...
export class AiToolGateway {
  ...
  async invoke({agent,context,taskId=null,toolName,input={}}){
    const tool=this.tools.get(toolName);if(!tool)throw new Error(`Unknown AI tool: ${toolName}`);
  ...
  async executeApproved({approval,agent,context}){
    ...
    if(!(agent.allowedTools??[]).includes(tool.name))throw new Error("agent no longer has permission for approved tool");
    if(tool.risk==="prohibited")throw new Error("prohibited AI tools cannot be executed even after approval");
