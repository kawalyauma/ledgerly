...
const TOOL_META=Object.freeze({
 ...
 create_document_draft:{description:"Create a persistent structured editable AI-authored document draft with permanent provenance.",parameters:object({type:{type:"string",enum:["letter","circular","notice","memo","minutes","agenda","report","certificate","correspondence","contract","policy","visitor_note","replenishment_request","support_ticket","other"]},title:string("Document title."),content:{type:"object",description:"Structured editable document content."}},["type","title","content"])},
 ...
 generate_report:{description:"Create a persistent structured unpublished report draft.",parameters:object({title:string("Report title."),content:{type:"object",description:"Structured report content."}},["title","content"])},
...
export function registerCoreTools(gateway,services={}){
  ...
  register("create_document_draft",["documents:write"],services.createDocumentDraft??unavailable("create_document_draft"));
