...
export class AiDocumentEngine{
  ...
  async reviseAi({context,agent,taskId=null,documentId,content,reason=null}){
    ...
    return this.database.transaction(async(tx)=>{
      ...
      if(!current)throw new Error("document not found");if(!["draft","in_review","changes_requested"].includes(current.status))throw new Error("AI may only revise draft or review-state documents");
      ...
      await tx.query(`UPDATE ledgerly_ai.documents SET content=$1::jsonb,version=$2,ai_provenance=$3::jsonb,updated_at=now() WHERE document_id=$4 AND organization_id=$5`,[JSON.stringify(content),version,JSON.stringify(provenance),documentId,organizationId]);
      ...
      return {...current,content,version,ai_provenance:provenance};
    });
  }
  async reviseHuman({context,documentId,content,reason=null}){
    ...
    return this.database.transaction(async(tx)=>{
      ...
      await tx.query(`UPDATE ledgerly_ai.documents SET content=$1::jsonb,version=$2,ai_provenance=$3::jsonb,human_editors=human_editors || $4::jsonb,updated_at=now() WHERE document_id=$5 AND organization_id=$6`,[JSON.stringify(content),version,JSON.stringify(provenance),JSON.stringify([{user_id:context.userId,user_name:context.userName??context.userId,timestamp:actor.timestamp}]),documentId,organizationId]);
  ...
  async setStatus({context,documentId,status,reason=null}){if(!DOCUMENT_STATUSES.includes(status))throw new Error(`invalid document status ${status}`);const organizationId=org(context);const result=await this.database.query(`UPDATE ledgerly_ai.documents SET status=$1,updated_at=now() WHERE document_id=$2 AND organization_id=$3 RETURNING *`,[status,documentId,organizationId]);if(!result.rowCount)throw new Error("document not found");await this.audit?.write?.({organization_id:organizationId,actor_type:"human",actor_id:context.userId,action:`document.${status}`,entity_type:"document",entity_id:documentId,reason});return result.rows[0];}
