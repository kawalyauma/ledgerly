...
export default{name:'finance-journals',prefix:PREFIX,business:true,priority:95,enabled(config){return config.extensions?.['finance-api']?.journalsCutover==='node'},async handle({request,url,runtime}){try{const ext=runtime.extensions?.['finance-api'],api=ext?.journals,reversals=ext?.reversals;if(!api||!reversals)throw Object.assign(new Error('Finance journals runtime unavailable'),{status:503,code:'FINANCE_SELFHOST_NOT_READY'});const p=parts(url);
 ...
 if(request.method==='GET'&&p.length===2&&p[1]==='reversal-preview'){const x=await authenticateFinance(runtime,request,'journals:read');return ok({data:await reversals.preview(x.organizationId,p[0])})}
 ...
 if(request.method==='POST'&&p.length===2&&p[1]==='reverse'){const x=await authenticateFinance(runtime,request,'journals:write');return ok({data:await reversals.reverseJournal(x.organizationId,x.userId,p[0],await readJson(request))})}
