...
function hasDirect(principal,permission){if(principal.role==='owner'||principal.role==='admin')return true;const scopes=Array.isArray(principal.scopes)?principal.scopes:[];if(scopes.includes('*')||scopes.includes('school:*')||scopes.includes(permission))return true;if(scopes.includes('school:write')&&(permission.endsWith(':write')||permission.endsWith(':manage')||permission.endsWith(':devices')||permission.endsWith(':biometrics')))return true;return permission==='attendance:read'&&(scopes.includes('school:read')||scopes.includes('school:write'));}
