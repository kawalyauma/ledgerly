...
if(request.method==='POST'&&p[0]==='runs'&&p.length===3&&p[2]==='post')return ok({data:await api.linkPostedPayrollRun({principal:x,runId:p[1],body:await readJson(request),requestId})});
if(request.method==='POST'&&p[0]==='runs'&&p.length===3&&p[2]==='reverse')return ok({data:await api.reversePayrollRun({principal:x,runId:p[1],body:await readJson(request),requestId})});
...
if(request.method==='POST'&&p[0]==='components'&&p.length===1)return ok({data:await api.createPayrollComponent({principal:x,body:await readJson(request),requestId})},201);
...
if(request.method==='POST'&&p[0]==='inputs'&&p.length===1)return ok({data:await api.createPayrollInput({principal:x,body:await readJson(request),requestId})},201);
if(request.method==='POST'&&p[0]==='salary-payments'&&p.length===1)return ok({data:await api.recordSalaryPayment({principal:x,body:await readJson(request),requestId})},201);
if(request.method==='POST'&&p[0]==='salary-payments'&&p.length===3&&p[2]==='reverse')return ok({data:await api.reverseSalaryPayment({principal:x,paymentId:p[1],body:await readJson(request),requestId})});
