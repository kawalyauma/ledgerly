...
try{
  ...
  const catalog=await database.query(`SELECT module_key,active FROM app_modules WHERE module_key IN ('school-management','academics') ORDER BY module_key`);
  const catalogKeys=new Set(catalog.rows.filter(row=>row.active===true).map(row=>row.module_key));
  const moduleCatalogReady=catalogKeys.has('school-management')&&catalogKeys.has('academics');
  const moduleRows=await database.query(`SELECT organization_id,module_key,enabled FROM organization_modules WHERE module_key IN ('school-management','academics') ORDER BY organization_id,module_key`);
  const academicsEnabledOrganizations=new Set(moduleRows.rows.filter(row=>row.module_key==='academics'&&row.enabled===true).map(row=>row.organization_id)).size;
