...
test("resumed worker replays already executed approval instead of invoking tool twice",async()=>{
  const statuses=[];let gatewayInvocations=0;let generation=0;
  ...
  const database={query:async(sql)=>{
    ...
    if(text.includes("FROM ledgerly_ai.tasks"))return {rows:[task]};
  ...
  const worker=new AiWorker({
    ...
    taskService:{setStatus:async(input)=>{statuses.push(input);return input;}},
  ...
  assert.ok(statuses.some((item)=>item.status==="completed"));
});
