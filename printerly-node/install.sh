...
for f in agent.mjs lib.mjs pair.mjs; do install -o root -g root -m 0644 "$SCRIPT_DIR/src/$f" "/opt/printerly/src/$f"; done
...
echo "Printerly v1.2 installed. Edit /etc/printerly/config.json, pair with 'sudo printerly-pair 123456', then start with 'sudo systemctl enable --now printerly-node'."
