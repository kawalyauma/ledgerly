...
OFFSITE_STATUS="not_configured"; RESTIC_DATA_SNAPSHOT=""; RESTIC_MANIFEST_SNAPSHOT=""

write_manifest(){
  cat <<JSON | atomic_write "$MANIFEST"
{"format_version":2,"set_id":"$SET_ID","created_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","status":"valid","postgres":{"path":"${PG_PATH#$BACKUP_ROOT/}","sha256":"$PG_SHA"},"objects":{"path":"${OBJ_PATH#$BACKUP_ROOT/}","sha256_manifest":"$OBJ_SUMS_SHA"},"config":{"path":"${CFG_PATH#$BACKUP_ROOT/}","sha256":"$CFG_SHA","contains_secret_values":false},"nvr":{"configuration_in_config_backup":true,"catalog_metadata_in_postgresql":true,"video_archive_managed_separately":true},"offsite":{"status":"$OFFSITE_STATUS","provider":"restic","encrypted":true,"data_snapshot":"$RESTIC_DATA_SNAPSHOT","manifest_snapshot":"$RESTIC_MANIFEST_SNAPSHOT"},"verification":$VERIFY_JSON}
JSON
}
write_manifest
...
if [[ -n "${RESTIC_REPOSITORY:-}" ]]; then
  ...
  OFFSITE_STATUS="verified"; write_manifest
