...
SAFE_PATHS=(
  ...
  selfhost/pgbouncer/Dockerfile
  selfhost/pgbouncer/entrypoint.sh
  selfhost/redis/redis.conf
...
# Secrets are deliberately excluded. Never add .env.selfhost, private keys, credential files, or restic passwords here.
tar -czf "$PARTIAL" -C "$LEDGERLY_REPO_ROOT" -- "${SAFE_PATHS[@]}"
tar -tzf "$PARTIAL" | grep -Eq '(^|/)\.env\.selfhost$|\.pem$|\.key$|credentials|secret' && die "unsafe secret-like file detected in config archive"
...
{"kind":"configuration","set_id":"$SET_ID","created_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","bytes":$SIZE,"sha256":"$SHA","secret_values_included":false,"nvr_configuration_included":true,"catalog_metadata_location":"postgresql"}
JSON
printf '%s\n' "$OUT"
