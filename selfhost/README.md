...
The first milestone provisions only shared infrastructure:
...
The existing Cloudflare Worker, D1, R2, queues, and cron schedules are deliberately untouched.
...
PostgreSQL, PgBouncer, Redis, and MinIO are published only on `127.0.0.1` during the migration so host-side development tools can reach them without exposing them to the LAN or internet. The internal Docker backend network is marked internal.

Caddy is the only service intended to become internet-facing. The current Caddy configuration exposes a health endpoint and deliberately returns 503 for application traffic until the self-hosted API/web runtime is introduced and validated.
