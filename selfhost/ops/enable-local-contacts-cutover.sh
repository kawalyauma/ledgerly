...
[[ "${LEDGERLY_RUNTIME_MODE:-$(read_env LEDGERLY_RUNTIME_MODE)}" != "production" ]] || fail "Keep LEDGERLY_RUNTIME_MODE=foundation during Contacts rehearsal"
...
run_schema(){ local phase="$1"; printf 'Preparing PostgreSQL phase: %s\n' "$phase"; "${DOCKER[@]}" compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" --profile migration run --rm migrate-d1 node src/migration/cli.mjs schema --phase "$phase"; }
run_schema auth-core
run_schema module-registry
run_schema mobile-sync-core
run_schema contacts

printf 'Validating Contacts schema, sync infrastructure and core module catalog...\n'
"${DOCKER[@]}" compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" --profile migration run --rm migrate-d1 node src/readiness/contacts-cutover-check.mjs

printf 'Enabling PostgreSQL module registry and Contacts authority...\n'
set_env LEDGERLY_PLATFORM_MODULES_SELFHOST_ENABLED true
set_env LEDGERLY_PLATFORM_MODULES_CUTOVER node
set_env LEDGERLY_CONTACTS_SELFHOST_ENABLED true
set_env LEDGERLY_CONTACTS_CUTOVER node

printf 'Restarting the self-hosted API...\n'
"${DOCKER[@]}" compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" up -d --build api
printf '\nContacts cutover rehearsal completed.\n'
printf '  Module registry: Node/PostgreSQL\n'
printf '  Contacts:        Node/PostgreSQL\n'
printf '  Runtime mode:    foundation\n'
printf '\nThe validator requires the core module catalog rows. For real cutover, migrate and validate module-registry, mobile-sync and Contacts data before switching authority.\n'
