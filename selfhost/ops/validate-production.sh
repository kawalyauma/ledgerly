...
if grep -Fq 'rateLimitMaxRequests' "$LEDGERLY_REPO_ROOT/server/src/index.mjs" && grep -Fq 'trustProxyHeaders' "$LEDGERLY_REPO_ROOT/server/src/index.mjs"; then PASS "rate limit and trusted-proxy handling"; else FAIL "rate limit and trusted-proxy handling"; fi
...
if grep -Fq 'minio-bootstrap:' "$LEDGERLY_COMPOSE_FILE" && grep -Fq 'ledgerly-app-policy.json' "$LEDGERLY_COMPOSE_FILE" && grep -Fq 'MINIO_APP_ACCESS_KEY' "$LEDGERLY_COMPOSE_FILE"; then PASS "bucket-scoped MinIO application bootstrap"; else FAIL "bucket-scoped MinIO application bootstrap"; fi
